...
class Space:
    ...
    def __init__(self, low, high, points):
        ...
        #                                   [1] * self._dimensions,
        #                                   points)
        self.__space = gen_all_binary_vectors(20)
    ...
    def search_point(self, point, k):
        # p_in = self.import_point(point).reshape(0, 1).astype('float64')
...
def gen_all_binary_vectors(length: int) -> torch.Tensor:
    return np.array(((torch.arange(2**length).unsqueeze(1) >> torch.arange(length-1, -1, -1)) & 1).float())
